...
namespace StevieDev.DialogueEditor
{
    public class LoadCSV
    {
        ...
        private void LoadInToNode(List<List<string>> result, List<string> headers, DialogueNodeData nodeData)
        {
            ...
            {
                if (line[0] == nodeData.SavedNodeGUID)
                {
                    for (int i = 0; i < line.Count; i++)
                    {
                        foreach (LanguageType languageType in (LanguageType[])Enum.GetValues(typeof(LanguageType)))
                        {
                            ...
                            {
                                nodeData.TextLanguages.Find(language => language.LanguageType == languageType).LanguageGenericType = line[i];
                            }
                        }
                    }
                ...
                if (line[0] == nodePort.PortGUID)
                {
                    for (int i = 0; i < line.Count; i++)
                    {
                        foreach (LanguageType languageType in (LanguageType[])Enum.GetValues(typeof(LanguageType)))
                        {
                            if (headers[i] == languageType.ToString())
                            {
                                nodePort.TextLanguages.Find(language => language.LanguageType == languageType).LanguageGenericType = line[i];
                            }
                        }
                    }
                }
        ...
        // Made by furukazu : https://github.com/furukazu/UnityCSVReader/edit/master/CSVReader.cs
